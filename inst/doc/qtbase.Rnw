...
@ 
%
In the constructor, one might notice the invocation of
\Rfunction{super}, which does not exist outside the scope of a
constructor (or method). Within a constructor, \Rfunction{super}
invokes the constructor of the parent (super) class, with the given
